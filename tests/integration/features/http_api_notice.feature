...
Feature: Notice HTTP API

@serial
Scenario: perform notice request within an advance procedure
    ...
    Then host server manager logs 'POST /notice HTTP/1.1'
    And client gets http response code 201
    And client gets notice response with index 0

@serial
Scenario: perform notice request with incorrect data within an advance procedure
    Given host server manager is up
    And dapp backend is up
    And host server manager has session with id `test_session`
    And client asks host server manager to advance with the following parameters:
...
@serial
Scenario: perform notice request without an advance procedure
    Given host server manager is up
    And dapp backend is up
    And host server manager has session with id `test_session`
    When client sends notice request with the following parameters:
